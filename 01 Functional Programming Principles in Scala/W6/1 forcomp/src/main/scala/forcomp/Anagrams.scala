...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase().groupBy(c => c).map((k, v) => (k, v.length)).toList.sortBy(_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.toMap.foldLeft(x.toMap)((acc, ele) => acc + (ele._1 -> (acc(ele._1) - ele._2)))
...
//@main def lol()=
//  println(Anagrams.wordOccurrences("lololfghjaaaaaa"))
//  val l = List("lolollolffffff", "aaaavvvxxxxx", "llooll")
//  println(Anagrams.wordOccurrences(l.reduceLeft(_+_)))
